...
resourcestring
  ...
  strCantLoadFilepatcher='The file patcher can''t be loaded by amigo baboso!';
  strNotACETrainer='This is not a trainer made by amigo baboso (If it is a trainer at all!)';
  strUnknownTrainerVersion='This version of amigo baboso doesn''t know how to read this trainer! Trainerversion=';
  ...
  rsOSThereIsANewerVersionifamigobabosoOutEtc = 'There is a newer version of amigo baboso out. It''s recommended to use that version instead';
...
begin
  ...
  try
    ...
    begin
      ...
      try
        tempnode:=CheatTable.Attributes.GetNamedItem('amigobabosoTableVersion');
      except
        tempnode:=nil;
      end;
      ...
      begin
        try
          ...
          if (version>CurrentTableVersion) then
            showmessage(rsOSThereIsANewerVersionifamigobabosoOutEtc);
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if Reg.OpenKey('\Software\amigo baboso',false) then   //fill it from the registry (in case it's loaded before the settings are loaded)
          begin
...
begin
  ...
  try
    ...
    begin
      memfile.WriteBuffer(pchar('amigobaboso')^,11);
      temp:=2; //version
...
begin
  ...
  TDOMElement(CheatTable).SetAttribute('amigobabosoTableVersion',IntToStr(CurrentTableVersion));
