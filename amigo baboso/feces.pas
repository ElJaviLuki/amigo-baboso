unit feces;
//friends endorsing amigo baboso system
...
resourcestring
  ...
  rsSelectYouramigobabosoSignatureFile = 'Select your amigo baboso signature '
    +'file';
  rsamigobabosoSignatureFiles = 'amigo baboso signature files';
  ...
  rsInvalidPublicKey = 'The provided public key is invalid(Not signed by the '
    +'amigo baboso guy). Remove the signature section to load this table';
  ...
  rsFailedToLoadamigobabosoPublicKey = 'Failed to load amigo baboso public key';
...
var
  amigobabosopublictablekey: BCRYPT_KEY_HANDLE=0;
...
begin
  ...
  try
    ...
    if amigobabosopublictablekey=0 then
    begin
      s:=BCryptImportKeyPair(hAlgoritm, 0, BCRYPT_ECCPUBLIC_BLOB, amigobabosopublictablekey, @publictablekey[0], 140, 0);
      if not succeeded(s) then raise exception.create(
        rsFailedToLoadamigobabosoPublicKey);
    end;
    ...
    s:=BCryptVerifySignature(amigobabosopublictablekey,nil,hashbuffer,hashlength,sig, signaturesize,0);
...
begin
  ...
  begin
    ...
    try
      od.Title:=rsSelectYouramigobabosoSignatureFile;
      od.Filter:=rsamigobabosoSignatureFiles+'|*.CESIG';
