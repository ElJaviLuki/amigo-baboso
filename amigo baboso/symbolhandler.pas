...
begin
  ...
  try
    ...
    symbolpath:=usedtempdir+'amigo baboso Symbols'+pathdelim;
...
var
  ...
  i,j: integer;
begin
  s:=amigobabosodir+'commonmodulelist.txt';
...
{$ifdef windows}
{$ifdef cpu32}
  dbghlp:=LoadLibrary(pchar(amigobabosoDir+'\win32\dbghelp.dll'));
{$else}
  dbghlp:=LoadLibrary(pchar(amigobabosoDir+'\win64\dbghelp.dll'));
{$endif}
...
begin
  ...
  databasepath:=usedtempdir+'amigo baboso Symbols'+pathdelim+'structures.sqlite';
end;
