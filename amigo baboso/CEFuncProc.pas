...
//amigo baboso regular WONT look at this
...
begin
  assignfile(memoryfile,amigobabosoDir+'Memory.TMP');
  assignfile(addressfile,amigobabosoDir+'Addresses.TMP');
  reset(memoryfile,1);
  reset(addressfile,1);

  assignfile(newmemoryfile,amigobabosoDir+'Memory2.TMP');
  assignfile(newaddressfile,amigobabosoDir+'Address2.TMP');
  ...
  deletefile(amigobabosoDir+'Memory.UNDO');
  deletefile(amigobabosoDir+'Addresses.UNDO');
  renamefile(amigobabosoDir+'Memory.tmp',amigobabosodir+'Memory.UNDO');
  renamefile(amigobabosoDir+'Addresses.tmp',amigobabosoDir+'Addresses.UNDO');
  renamefile(amigobabosoDir+'Memory2.tmp',amigobabosoDir+'Memory.TMP');
  Renamefile(amigobabosoDir+'Address2.TMP',amigobabosoDir+'Addresses.TMP');
...
//http://forum.amigobaboso.org/viewtopic.php?t=564382
{$IFDEF windows}
type
...
{$ENDIF}
begin
  amigobabosoDir:=ExtractFilePath(application.ExeName);
  result:=amigobabosoDir;
...
begin
  deletefile(amigobabosoDir+'Memory.TMP');
  deletefile(amigobabosoDir+'Addresses.TMP');
  deletefile(amigobabosoDir+'Memory.UNDO');
  deletefile(amigobabosoDir+'Addresses.UNDO');
  freemem(memory);
  memory:=nil;
...
begin
  ...
  try
    ...
    try
      ...
      if Reg.OpenKey('\Software\amigo baboso',false) then
      begin
        if reg.valueexists('Save window positions') then
          if reg.readbool('Save window positions') = false then exit;
      end;

      if Reg.OpenKey('\Software\amigo baboso\Window Positions '+inttostr(screen.PixelsPerInch),false) or Reg.OpenKey('\Software\amigo baboso\Window Positions',false) then
      begin
        s:=form.Name;
        s:=s+rsPosition;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\amigo baboso\Window Positions '+inttostr(screen.PixelsPerInch),true) then
      begin
...
begin
  result:=filename;
  if pos(uppercase(amigobabosoDir),uppercase(filename))=1 then
    result:='.\'+copy(filename,length(amigobabosodir)+1,length(filename));
end;
...
begin
  ...
  path:=path+'amigo baboso Symbols';
