...
begin
  ...
  begin
    ...
    MainForm.frmLuaTableScript.assemblescreen.Lines.Insert(2, '  messageDialog(''Please install amigo baboso ''..RequiredCEVersion, mtError, mbOK)');
  ...
  try
    ...
    if CopyFile(amigobabosodir+basefile+'.dat', filename) then
    begin
      ...
      begin
        ...
        begin
          ...
          decompressor.LoadFromFile(amigobabosodir+'standalonephase2.dat');
          ...
          addfile(amigobabosodir+'defines.lua');

          if rb32.checked then
          begin
            addfile(amigobabosodir+'amigobaboso-i386.exe');

            addfile(amigobabosodir+'lua53-32.dll');
            addfile(amigobabosodir+'win32\dbghelp.dll','win32');

            if cbSpeedhack.checked then
              addfile(amigobabosodir+'speedhack-i386.dll');

            if cbvehdebug.checked then
              addfile(amigobabosodir+'vehdebug-i386.dll');

            if cbKernelDebug.checked then
            begin
              addfile(amigobabosodir+'dbk32.sys');
              addfile(amigobabosodir+'dbk64.sys');
              addfile(amigobabosodir+'amigobaboso-i386.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(amigobabosodir+'libmikmod32.dll');

          end
          else
          begin
            addfile(amigobabosodir+'amigobaboso-x86_64.exe');
            addfile(amigobabosodir+'lua53-64.dll');

            if cbSpeedhack.checked then
              addfile(amigobabosodir+'speedhack-x86_64.dll');

            if cbvehdebug.checked then
              addfile(amigobabosodir+'vehdebug-x86_64.dll');

            if cbKernelDebug.checked then
            begin
              addfile(amigobabosodir+'dbk64.sys');
              addfile(amigobabosodir+'amigobaboso-x86_64.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(amigobabosodir+'libmikmod64.dll');

          end;

          if cbDotNet.checked then
          begin
            addfile(amigobabosodir+'DotNetDataCollector32.exe');
            addfile(amigobabosodir+'DotNetDataCollector64.exe');
          end;

          if cbD3DHook.checked then
          begin
            addfile(amigobabosodir+'overlay.fx');
            if rb32.checked then
            begin
              addfile(amigobabosodir+'d3dhook.dll');
              addfile(amigobabosodir+'ced3d9hook.dll');
              addfile(amigobabosodir+'ced3d10hook.dll');
              addfile(amigobabosodir+'ced3d11hook.dll');
            end
            else
            begin
              addfile(amigobabosodir+'d3dhook64.dll');
              addfile(amigobabosodir+'ced3d9hook64.dll');
              addfile(amigobabosodir+'ced3d10hook64.dll');
              addfile(amigobabosodir+'ced3d11hook64.dll');
            end;
          end;
      ...
      showMessage(Format(rsTrainerFileMissing, [amigobabosodir+basefile, filename]));
...
begin
  ...
  d:=ExtractRelativepath(amigobabosodir, d);
...
begin
  ...
  OpenDialog1.InitialDir:=amigobabosoDir;
  SelectDirectoryDialog1.InitialDir:=amigobabosoDir;
