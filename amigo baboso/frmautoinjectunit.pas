...
{
The TDisassemblyLine originates from jgoemat  ( http://forum.amigobaboso.org/viewtopic.php?t=566415 )
...
resourcestring
  ...
  rsCEAFilter = 'amigo baboso Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = 'amigo baboso GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
begin
  ...
  begin
    ...
    try
      if reg.OpenKey('\Software\amigo baboso\Auto Assembler\',false) then
      begin
...
{$ifndef standalonetrainerwithassembler}
 ...
 obsolete
  ...
  begin
    ...
    injectdll(amigobabosodir+'undercdll.dll','');
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\amigo baboso\Auto Assembler\',true) then
          begin
...
// \/   http://forum.amigobaboso.org/viewtopic.php?t=566415 (jgoemat and some mods by db)
procedure GenerateFullInjectionScript(Script: tstrings; address: string; commentRadius: integer=10);
var
  originalcode: array of string;
  originalbytes: array of byte;
...
begin
  ...
  // same as menu option "amigo baboso framework code", make sure we
...
begin
  LuaHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\amigo baboso\Lua Highlighter'+darkmodestring);
  AAHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\amigo baboso\AA Highlighter'+darkmodestring);
end;
...
begin
  ...
  begin
    LuaHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\amigo baboso\Lua Highlighter'+darkmodestring);
    reloadHighlighterSettings;
...
begin
  ...
  if frmHighlighterEditor.showmodal=mrok then
  begin
    AAHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\amigo baboso\AA Highlighter'+darkmodestring);
