...
type
  TLuaSettings=class    //A wrapper for the registry object to make access to the amigo baboso settings easier and uniform
  private
    freg: Tregistry;
    fpath: string;
...
begin
  ...
  if freg.OpenKey('\Software\amigo baboso\'+v, true) then
    fpath:=v;
end;
...
begin
  ...
  if initialpath<>nil then
    ...
    freg.OpenKey('\Software\amigo baboso',true);
