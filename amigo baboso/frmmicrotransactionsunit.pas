...
begin
  ...
  if internet=nil then
    internet:=TWinInternet.Create('amigo baboso microtransaction system');

  ss:=tstringstream.create({$if FPC_FULLVERSION<030200}''{$endif});
  try
    try
      internet.getURL('https://amigobaboso.org/microtransaction.php?action=buy&amount='+inttostr(tbitbtn(sender).Tag), ss);
      ...
      MessageDlg('Sorry, but the amigo baboso server is currently overloaded or you have no internet. Please try again later',mtInformation, [mbok],0);
    end;
  finally
    ss.free;
  end;
