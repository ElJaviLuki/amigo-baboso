...
begin
  ...
  begin
    ...
    begin
      ...
      if filename='amigobaboso-i386.exe' then
        filename:=ExtractFileName(GetModuleName(0)); //give it the same name as the trainer

      if filename='amigobaboso-x86_64.exe' then
        filename:=ExtractFileName(GetModuleName(0)); //give it the same name as the trainer

      if filename='amigobaboso-i386.exe.sig' then
        filename:=ExtractFileName(GetModuleName(0))+'.sig';

      if filename='amigobaboso-x86_64.exe.sig' then
        filename:=ExtractFileName(GetModuleName(0))+'.sig';
