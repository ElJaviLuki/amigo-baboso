...
begin
  try
    f:=TFilestream.Create(amigobabosoDir+'accesscheck.tmp', fmCreate);
    try
  ...
  try
    f:=TFilestream.Create(amigobabosoDir+'accesscheck.tmp', fmOpenReadWrite);
    try
      f.Seek(0,soFromEnd);
  ...
  if not deletefile(amigobabosoDir+'accesscheck.tmp') then
