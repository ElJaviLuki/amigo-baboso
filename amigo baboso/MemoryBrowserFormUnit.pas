...
resourcestring
  ...
  rsamigobabosoSingleLingeAssembler = 'Single-line assembler';
  ...
  rsPleaseTargetAnotherProcess = 'Start another version of amigo baboso and attach to that instead';
...
begin
  ...
  try
    if reg.OpenKey('\Software\amigo baboso\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',true) then
    begin
    ...
    if reg.OpenKey('\Software\amigo baboso\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,true) then
      SaveFontToRegistry(disassemblerview.font, reg);

    if reg.OpenKey('\Software\amigo baboso\Hexview '+inttostr(screen.PixelsPerInch),true) then
    begin
      reg.writeInteger('spaceBetweenLines', hexview.spaceBetweenLines);
      reg.WriteBool('showStatusBar', hexview.statusbar.Visible);
    end;

    if reg.openkey('\Software\amigo baboso\Hexview'+darkmodestring,true) then
    ...
    if reg.OpenKey('\Software\amigo baboso\Hexview '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,true) then
      SaveFontToRegistry(hexview.hexfont, reg);

    if reg.OpenKey('\Software\amigo baboso\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,true) then
...
begin
  ...
  reg:=Tregistry.Create;
  try
    if reg.OpenKey('\Software\amigo baboso\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,false) then
    begin
      LoadFontFromRegistry(f, reg);
      disassemblerview.font:=f;
    end;

    if reg.OpenKey('\Software\amigo baboso\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',false) then
    begin
    ...
    if reg.OpenKey('\Software\amigo baboso\Hexview '+inttostr(screen.PixelsPerInch),false) then
    begin
    ...
    if reg.OpenKey('\Software\amigo baboso\Hexview '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,false) then
    begin
      LoadFontFromRegistry(f, reg);
      hexview.hexfont:=f;
    end;

    if reg.OpenKey('\Software\amigo baboso\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font'+darkmodestring,false) then
    begin
      LoadFontFromRegistry(f, reg);
//      scrollbox1.Font:=f;
...
begin
  ...
  begin
    ...
    begin
      beep; //Best sound effect amigo baboso has
      exit;
    end;
  end;
...
//  copy
  ...
  assemblercode:=InputboxTop(rsamigobabosoSingleLingeAssembler, Format(rsTypeYourAssemblerCodeHereAddress, [inttohex(Address, 8)]), assemblercode, x='', canceled{$ifndef darwin},assemblerHistory{$endif});
...
{will save a amigo baboso memory region file .CEM}
begin
