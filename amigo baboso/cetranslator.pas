...
{
Modified for amigo baboso use because filenames differ between 32 and 64-bit (also different po file, but just pick the one with the most data...)
...
var
  ...
  begin
    ...
    begin
      ...
      Result := amigobabosodir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'amigobaboso'+LCEXT;
      if FileExists(Result) then exit;

      Result := amigobabosodir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'amigobaboso-x86_64'+LCEXT;
      if FileExists(Result) then exit;

      Result := amigobabosodir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'amigobaboso-i386'+LCEXT;
      if FileExists(Result) then exit;
...
begin
  ...
  if (lang = '') and (FileExists(amigobabosodir+ 'languages' + DirectorySeparator+'language.ini')) then
  begin
    try
      lini:=TIniFile.Create(amigobabosodir+'languages' + DirectorySeparator+'language.ini');
      try
