...
begin
  ...
  begin
    ...
    if (cpuid7ebx and (1 shl 5))>0 then
      exename:='amigobaboso-x86_64-SSE4-AVX2.exe'
    else
      exename:='amigobaboso-x86_64.exe';
  end;
  ...
  begin
    s:=exename;
    MessageBoxA(0, pchar(s+' could not be found. Please disable/uninstall your anti virus and reinstall amigo baboso to fix this'),'amigo baboso launch error',MB_OK or MB_ICONERROR);
  end;
