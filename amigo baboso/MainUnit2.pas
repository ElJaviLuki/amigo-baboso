    ...
    try
      ...
      begin
        ...
        begin
          ...
          if reg.ValueExists('Show amigo baboso Hotkey') then
            {$ifdef windows}
            reg.ReadBinaryData('Show amigo baboso Hotkey',temphotkeylist[1][0],10);
            {$else}
            HexToBin(pchar(reg.ReadString('Show amigo baboso Hotkey')),pchar(@temphotkeylist[1][0]),10);
            {$endif}
      ...
      if Reg.OpenKey('\Software\amigo baboso\Tools',false) then
      begin
      ...
      if (not skipPlugins) and (Reg.OpenKey('\Software\amigo baboso\Plugins'{$ifdef cpu64}+'64'{$else}+'32'{$endif},false)) then
      begin
        names:=TStringList.create;
        try
