...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure Closeamigobaboso1Click(Sender: TObject);
...
resourcestring
  ...
  rsThankYouForTryingOutamigobabosoBecauseItHasExpired =
    'Thank you for trying out amigo baboso. Because it has expired amigo baboso will now close. Is that ok with you?';
  ...
  strClickToGoHome = 'Click here to go to the amigo baboso homepage';
  ...
  rsHex = 'Hex';
  rsDoYouWantToGoToTheamigobabosoWebsite =
    'Do you want to go to the amigo baboso website?';
  ...
  rsBringsamigobabosoToFront = 'brings amigo baboso to front';
  ...
  strfuture = 'Wow,I never imagined people would use amigo baboso up to today';
  rsLicenseExpired =
    'Your license to use amigo baboso has expired. You can buy a license to use amigo baboso for 1 month for $200, 6 months for only $1000 and for 1 year for ' + 'only $1800. If you don''t renew your license amigo baboso will be severely limited in it''s abilities. (e.g: Next scan has been disabled)';
  rsEXPIRED = 'EXPIRED';
  strdontbother =
    'Don''t even bother. amigo baboso uses the main thread to receive messages when the scan is done, freeze it and CE will crash!';
  ...
  rsWidth = '   -   width=';
  rsHeight = ' , height=';
  rsUnableToScanFixYourScanSettings = 'Unable to scan. Fix your scan settings and restart amigo baboso';
...
//  symhandler.waitforsymbolsloaded;
  ...
  begin
    if fileexists(TablesDir +  pathdelim + expectedfilename) or fileexists(expectedfilename) or
      fileexists(amigobabosodir + expectedfilename) then
    begin
      ...
      begin
        ...
        if fileexists(expectedfilename) then
          ...
          opendialog1.FileName := amigobabosodir + expectedfilename;
...
begin
  ...
  p.Execute;
  {$else}
  shellexecute(0, 'open', pchar(amigobabosodir+'Tutorial-x86_64.exe'), nil, nil, sw_show);
  {$endif}
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\amigo baboso\FoundList'+darkmodestring, True) then
      begin
...
begin
  ...
  try
    if reg.OpenKey('\Software\amigo baboso\CustomTypes\', False) then
    begin
      ...
      try
        ...
        begin
          if reg.OpenKey('\Software\amigo baboso\CustomTypes\' + CustomTypes[i], False) then
          begin
            try
              islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\amigo baboso\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\amigo baboso\CustomTypes\' + oldname);
        freeandnil(reg);
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\amigo baboso\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  ...
  if InputQuery(rsamigobaboso, rsWhatWillBeTheNewNameForThisTab, s) then
...
begin
  if aprilfools then
  begin
    if messagedlg(rsThankYouForTryingOutamigobabosoBecauseItHasExpired,
...
begin
  ...
  wikipath:='https://wiki.amigobaboso.org/index.php';
  wikiurl:='';
  ...
  begin
    case data of
      ...
      19:   wikiurl:='?title=amigo_baboso:Lua';
      1089: wikiurl:='?title=amigo_baboso:Auto_Assembler';
    end;

    {$ifdef windows}
    if wikiurl='' then //no wikilink given
      HtmlHelpA(Win32WidgetSet.AppHandle, PChar(amigobabosodir + 'amigobaboso.chm'), HH_HELP_CONTEXT, Data)
    else
    {$endif}
...
var s: string;
begin
  s:=format('http://www.amigobaboso.org/?referredby=CE%.2f',[ceversion]);
  if messagedlg(rsDoYouWantToGoToTheamigobabosoWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
...
begin
  ...
  begin
    ...
    fronttext := rsBringsamigobabosoToFront;
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\amigo baboso', False) then //can't be opened. Clean install
    begin
      if Reg.OpenKey('\Software\amigo baboso', True) then
      begin
...
{  if aprilfools = True then
  ...
  if Reg.OpenKey('\Software\amigo baboso\FoundList'+darkmodestring, false) then
  begin
...
procedure TMainForm.Closeamigobaboso1Click(Sender: TObject);
begin
  Close;
end;
...
begin
  shellexecute(0, 'open', pchar(amigobabosodir+'Tutorial-i386.exe'), nil, nil, sw_show);
end;
...
begin
  ShellExecute(0,'open','https://wiki.amigobaboso.org/index.php',nil,nil,SW_SHOW);
