...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in amigo baboso finding NO memory! Are you stupid?';
  ...
  rsPopupHideamigobaboso = 'Popup/Hide amigo baboso';
  ...
  rsRestartCE = 'It is recommended to restart amigo baboso for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.{$ifdef windows}WriteBinaryData{$else}WriteString{$endif}('Show amigo baboso Hotkey',{$ifndef windows}bintohexs({$endif}frameHotkeyConfig.newhotkeys[1][0],10){$ifndef windows}){$endif};
        ...
        begin
          if reg.OpenKey('\Software\amigo baboso\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\amigo baboso\Tools');
      if Reg.OpenKey('\Software\amigo baboso\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\amigo baboso\Plugins'+cpu);
      if Reg.OpenKey('\Software\amigo baboso\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    ...
    try
      ini:=TIniFile.Create(amigobabosodir+'languages' + DirectorySeparator+'language.ini');
      try
...
begin
  ...
  FindAllDirectories(f,amigobabosoDir+'\languages',false);
  for i:=0 to f.Count-1 do
  begin
    n:=f[i];
    if not (fileexists(n+'\amigobaboso.po') or fileexists(n+'\amigobaboso-x86_64.po') or fileexists(n+'\amigobaboso-i386.po')) then
      continue;
