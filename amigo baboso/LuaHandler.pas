...
resourcestring
  ...
  rsamigobabosoIsBeingAFag = 'amigobaboso is being a fag';
...
function lua_ToCEUserData(L: PLua_state; i: integer): pointer;
//amigo baboso implements two types of userdata. the legacy LightUserData used in 6.2- and the Heavy UserData in 6.3+
          ...
          begin
            case parameters[i].VType of
              ...
              system.vtWideChar, vtPWideChar, vtVariant, vtInterface,
                vtWideString: lua_pushstring(L, rsamigobabosoIsBeingAFag);
...
function supportamigobaboso(L: Plua_State): integer; cdecl;
var
  parameters: integer;
...
function fuckamigobaboso(L: Plua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
  if adwindow<>nil then
...
function getamigobabosoDir(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
  lua_pushstring(L, amigobabosoDir);
  result:=1;
end;

function lua_getamigobabosoProcessID(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
...
function amigobabosoIs64Bit(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
...
function lua_getamigobabosoFileVersion(L: Plua_State): integer; cdecl;
begin
...
begin
  ...
  begin
    ...
    try
      cefuncproc.InjectDll(amigobabosoDir+'winhook-'+ts+'.dll');
    except
    end;
  end;
...
begin
  ...
  lua_register(L, 'amigobabosoIs64Bit', amigobabosoIs64Bit);
...
begin
  ...
  begin
    ...
    Lua_register(L, 'supportamigobaboso', supportamigobaboso);
    Lua_register(L, 'fuckamigobaboso', fuckamigobaboso);
    ...
    lua_register(L, 'getamigobabosoDir', getamigobabosoDir);
    lua_register(L, 'getamigobabosoProcessID', lua_getamigobabosoProcessID);
    ...
    lua_register(L, 'getamigobabosoFileVersion', lua_getamigobabosoFileVersion);
...
{$endif}
      ...
      {$else}
      autorunpath:=amigobabosoDir+'autorun'+pathdelim;
      {$endif}
