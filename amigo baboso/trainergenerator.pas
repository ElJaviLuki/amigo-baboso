...
type
  ...
  TfrmTrainerGenerator = class(TForm)
    ...
    cbSupportamigobaboso: TCheckBox;
    ...
    procedure cbSupportamigobabosoChange(Sender: TObject);
...
resourcestring
  ...
  rsTo = 'to';
  rsBy = 'by';
  rsOnCloseWarning = 'This form had an onClose event. Good thing this was only a stub, else amigo baboso would have terminated';
  ...
  rsYouNeedACheatTableWithCheatEntries = 'You need a cheat table with cheat '
    +'entries';
  rsDonTSupportamigobabosoOrYourself = 'Don''t support amigo baboso (or '
    +'yourself)';
...
begin
  ...
  if not cbSupportamigobaboso.checked then
    cbSupportamigobaboso.checked:=true;
...
begin
  ...
  l.add('  messageDialog(''Please install amigo baboso ''..RequiredCEVersion, mtError, mbOK)');
  l.add('  closeCE()');
  l.add('end');
  ...
  try
    ...
    begin
      ...
      begin
        ...
        l.add('supportamigobaboso('+trainerform.name+', '+BoolToStr(adconfig.cbCanClose.checked,'true','false')+', '+adconfig.edtWidth.text+', '+adconfig.edtHeight.text+', '+inttostr(adconfig.adposition)+', '+QuotedStr(adconfig.ownurl)+', '+QuotedStr(adconfig.extraparam)+', '+inttostr(adconfig.percentage)+')');
...
begin
  cbSupportamigobaboso.caption:=rsDonTSupportamigobabosoOrYourself;
  if restoretimer<>nil then
    restoretimer.enabled:=false;
end;

procedure TfrmTrainerGenerator.cbSupportamigobabosoChange(Sender: TObject);
begin
  //Guilt procedure
  if not cbSupportamigobaboso.checked then
  begin
    cbSupportamigobaboso.caption:=rsThankYou;
  ...
  begin
    cbSupportamigobaboso.caption:=rsAaaaw;
    if adwindow<>nil then
    begin
